...
def readScenariosData():
    ...
    try:
           connection = psycopg2.connect(
                                          ...
                                          database="geodb")
           cursor = connection.cursor()           
           postgreSQL_Update_Query = (f"UPDATE spd8r.usa_va_mrdd_scenarios SET end_date=NOW() WHERE name IN (SELECT name FROM {data_table} WHERE name NOT IN ({scenarioNames[:-2]}) AND end_date IS NULL);")
